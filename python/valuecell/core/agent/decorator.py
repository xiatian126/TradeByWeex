        ...
        class DecoratedAgent(cls):
            ...
            async def serve(self):
                ...
                try:
                    ...
                    # uvicorn may raise SystemExit(1) when port is in use
                    if isinstance(e, OSError) and e.errno == 48:
                        logger.warning(
                            f"Port {self._port} already in use for {agent_name}, "
                            "assuming server is already running. This is expected if "
                            "multiple strategies share the same agent."
                        )
                        # Don't re-raise - allow the connection to proceed
                        # The agent server might already be running
                    elif isinstance(e, SystemExit) and e.code == 1:
                        # uvicorn calls sys.exit(1) on port conflict
                        logger.warning(
                            f"Port {self._port} conflict for {agent_name} (SystemExit), "
                            "assuming server is already running."
                        )
                        # Don't re-raise SystemExit - convert to return
                        return
                    else:
